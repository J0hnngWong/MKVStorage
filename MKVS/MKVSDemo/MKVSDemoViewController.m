...
@property (nonatomic, strong) MKVWStorageManager *mkvmStorageManager;
...
- (void)viewDidLoad {
    ...
    [self.mkvmStorageManager printFilePath];
    self.times = 0;
...
- (IBAction)saveString:(id)sender {
    ...
    [self.mkvmStorageManager setStringValue:@"string" forKey:@"string"];
}
...
- (MKVWStorageManager *)mkvmStorageManager
{
    if (_mkvmStorageManager == nil) {
        _mkvmStorageManager = [[MKVWStorageManager alloc] init];
    }
    return _mkvmStorageManager;
}
